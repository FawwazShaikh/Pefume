...
"            {filteredAndSortedItems.map((item) => (\n              <div\n                key={item.id}\n                onClick={() => handleProductClick(item.id)}\n                className=\"\n                  group h-full flex flex-col bg-[#FAF8F4] rounded-[16px]\n                  border border-[#E8E2D8] shadow-[0_4px_20px_rgba(31,31,31,0.03)] \n                  hover:shadow-[0_12px_30px_rgba(176,141,87,0.08)]\n                  transition-all duration-300 ease-out hover:-translate-y-1 overflow-hidden cursor-pointer\n                \"\n              >\n                {/* Product Image and Badges */}\n                <div className=\"relative aspect-[4/5] w-full overflow-hidden bg-[#FAF8F4] flex items-center justify-center border-b border-[#E8E2D8]\">\n                  <img\n                    src={item.image}\n                    alt={item.name}\n                    loading=\"lazy\"\n                    className=\"w-full h-full object-cover object-center transition-transform duration-300 ease-out group-hover:scale-[1.03]\"\n                  />\n\n                  {/* Overlays Badges */}\n                  <div className=\"absolute top-3 left-3 right-3 z-10 flex flex-col gap-1 pointer-events-none items-start\">\n                    {item.tags && item.tags.includes('featured') && (\n                      <span className=\"text-[0.45rem] font-bold tracking-widest uppercase bg-[#1F1F1F] text-white px-2.5 py-1 rounded-sm shadow-sm\">\n                        FEATURED\n                      </span>\n                    )}\n                    {item.tags && item.tags.includes('new-arrival') && (\n                      <span className=\"text-[0.45rem] font-bold tracking-widest uppercase bg-[#B08D57] text-white px-2.5 py-1 rounded-sm shadow-sm\">\n                        NEW ARRIVAL\n                      </span>\n                    )}\n                    {item.tags && item.tags.includes('out-of-stock') && (\n                      <span className=\"text-[0.45rem] font-bold tracking-widest uppercase bg-[#FF
