...
function deepUnstringify(obj) {
  if (typeof obj === 'string') {
    let un = unstringify(obj);
    if (un !== obj) return deepUnstringify(un);
    return obj;
  } else if (Array.isArray(obj)) {
    return obj.map(deepUnstringify);
  } else if (obj && typeof obj === 'object') {
    let res = {};
    for (let k in obj) {
      res[k] = deepUnstringify(obj[k]);
    }
    return res;
  }
  return obj;
    ...
    if (entry.tool_calls) {
      for (const call of entry.tool_calls) {
        ...
        if (name === 'write_to_file' || name === 'replace_file_content' || name === 'multi_replace_file_content') {
          ...
          let parsedArgs = deepUnstringify(args);
